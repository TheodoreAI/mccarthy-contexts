...
transfer is \emph{route-dependent}, failing between compatible endpoints when
passed through an intermediary. All 21 results are machine-checked in Lean 4 and
free of \texttt{sorry}. The development is available at
...
For every ordinal $\alpha$, the tower extended to stage $\alpha$ is consistent.
\end{corollary}
\leanref{TranscendenceTower.Transfinite.Gamma\_satisfiable}
...
$\Gamma$ is categorical over the base: any two contexts extending the same
level-0 valuation and satisfying $\Gamma$ are equal.
\end{theorem}
...
$p$ transfers to $c_3$ determinately. Through $c_2$, two incomparable minimal
resolutions appear and $p$ survives only one.
\end{theorem}
\leanref{TranscendenceTower.Chaining.lifting\_does\_not\_chain}
...
the limit-stage lemma would be quietly empty.
