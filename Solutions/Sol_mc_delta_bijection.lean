...
theorem solution (v : Atom 0 → Bool) (A : Set (Lang 0)) :
    (∀ N₁ ∈ Mod v A, ∀ N₂ ∈ Mod v A, Delta v N₁ = Delta v N₂ → N₁ = N₂)
      ∧ (∀ S : Set (Lang 0), S ⊆ A → ∃ N ∈ Mod v A, Delta v N = S) := by
  refine ⟨fun N₁ h₁ N₂ h₂ hD => delta_inj' h₁ h₂ hD, fun S hS => ?_⟩
  obtain ⟨N, hN, hD⟩ := delta_surj' v A S hS
  exact ⟨N, hN, hD⟩
